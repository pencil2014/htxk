...
var commonPublish = {
	...
	// 点击弹框
	slideDialog: function(thatDom) {
		$('.manage_list_menu.phblish_btn').on('click', 'li', function(event) {
			event.preventDefault();
			var that=this;
			var aueditor=thatDom.articleUEditor;
				if((aueditor && aueditor.getContent().replace(/_ueditor_page_break_tag_/g,"").length>0 )
						|| ($('#imagesList') && $('#imagesList').find('li').length>1)
						|| ($('#start_video_div').hasClass('webuploader-element-invisible'))){
					layer.open({
						scrollbar: false,
						move: false,
						...
						yes: function(index, layero) {
							window.location=$(that).find('a').attr('data-href');
						},
					});
				}
				else{ //如果内容没有变化，直接切换
	...
	// 定时发布弹框
	timeDialog:function(publishType,thatDom){
		$.hhly.ajax({
			url:'/time/timestamp',
			type:'get',
			success:function(data){
						...
						$.hhly.ajax({
							...
							success:function(data){
								...
								else
								{
									if(publishType==1){
										var aueditor=thatDom.articleUEditor;
										thatDom.formVaridate(0,true,aueditor);
									}else if(publishType==2){
										thatDom.formVaridate(0,true);
									}
								}
							}
						});
	...
	timePublish:function(thatDom){
		...
		$("body").on('click','.publish_options .btn_publish',function(){
			...
			commonPublish.timeDialog(publishType,thatDom);
		});
	},
	// 字数统计
	...
	init: function(thatDom) {
		this.slideDialog(thatDom);
		this.timePublish(thatDom);
	}
};
