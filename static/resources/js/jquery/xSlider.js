;(function($){
  ...
  var xSlider = function(el, userConfig) {
    ...
    // 保存查找dom元素
    var slider_img = this.el.children('.slide_wrap')
    var slider_img_ul = slider_img.children('ul')
    var slider_img_ul_li = slider_img_ul.children('li');
    var firstLi=slider_img_ul_li.eq(0).clone();
    var lastLi=slider_img_ul_li.eq(slider_img_ul_li.length-1).clone();
    slider_img_ul.prepend(lastLi);
    slider_img_ul.append(firstLi);
    slider_img_ul = slider_img.children('ul')
    slider_img_ul_li = slider_img_ul.children('li');
    var winWidth=$(window).width();
    this.el.width(winWidth)
    slider_img_ul.width(winWidth*slider_img_ul_li.length);
    slider_img_ul_li.width(winWidth);
    ...
    // 初始化圆点
    this.el.append('<div class="pagination"><ul></ul></div>')
    var slider_dot = this.el.children('.pagination')
