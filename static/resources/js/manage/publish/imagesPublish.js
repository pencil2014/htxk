var imageEdit={
	...
	// input初始化
	inputInit:function(){
		$('div[name="bigImagesCategory"],div[name="smallImagesCategory"]').inputbox({height:24,width:110});
	},
	typeSuccess:function(data){
		var optionHtml = "";
		var cloneHtml = $('#imagesSmallCategory .opts').find('a').first().clone();
		for (var i = 0; i < data.length; i++) {
			optionHtml += '<a href="javascript:;" val="' + data[i].cateId + '">' + data[i].cateName + '</a>';
		}
		$('#imagesSmallCategory .opts').empty().append(cloneHtml).append(optionHtml);
	},
	// 下拉选择初始化
	selectInit:function(){
		$('#imagesBigCategory').on('click','a',function(){
			var parentId = $(this).attr("val");
			var test=$('#imagesSmallCategory .opts').find('a').first().text();
			$('#imagesSmallCategory .selected').text(test);
			if(parentId != 0){
				var typeUrl='/manage/news/getCategoryArticleList?parentId='+parentId;
				$.hhly.ajax({url:typeUrl,success:imageEdit.typeSuccess});
			}else{
				var cloneTips=$('#imagesSmallCategory .opts').find('a:first').clone();
				$('#imagesSmallCategory .opts').empty().append(cloneTips);
			}
		});
	},
	//切换提示
	...
	imageSave:function(type, timingPublish,saveCallback){
		...
		var cateId = $('#imagesSmallCategory .opts a[class="selected"]').attr("val");
		var content = "";
    	...
    	covers:covers,plainpublishtime:plainpublishtime,publishStatus:status};
	...
	textAccount:function(){
		$('#manage_contents').off('input propertychange', '.imagesShowWordNum');
		$('#manage_contents').on('input propertychange', '.imagesShowWordNum', function() {
			commonPublish.videoShowWordNum(this,120);
		});
		/*重新编辑内容的的时候需要用到 初始化*/
	...
	reload:function(){
		...
		$('#imagesBigCategory opts').find('a').first().trigger('click.input');
