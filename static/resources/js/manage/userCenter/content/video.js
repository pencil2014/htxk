...
require(['jquery','template','paging', 'hhlyAction', 'layer','common','i18n','personal'],function($,template,Paging){
	var video={
		pageInt:function(total,page){
			...
			var status=$('ul.title_list li.publishActive').index();
			pageDom.init({
				target:'.page_box',
				pagesize:10,
				count:total,
				...
				callback:function(page,size,count){
					$.hhly.ajax({
						...
						success:function(data){
							...
							$('.share_video>ul').empty().append(html);
							$(window).scrollTop(0);
						}
					})
				}
		...
		videoSlide:function(){
			$('ul.title_list li').click(function(){
				...
				$.hhly.ajax({
					...
					success:function(data){
						var html=template('video_tpl',data);
						$('.share_video').empty().append(html);
						$(window).scrollTop(0);
						$(that).addClass('publishActive').siblings().removeClass('publishActive');
						$('.page_box').empty();
						if(data.data.hasNextPage){
							video.pageInt(data.data.total,data.data.page);
						}
					}
				})
			});
		...
		init:function(){
			this.videoSlide();
			$('ul.title_list li').eq(0).trigger('click');
			this.deleteItem();
			this.rebackItem();
		}
	}
