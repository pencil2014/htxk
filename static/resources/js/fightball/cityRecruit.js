...
define(['jquery', 'template', 'sort', 'nav', 'app','cityCommon','blockUI','common','i18n'], function ($, template, sort, nav, app,cityCommon,blockUI) {
    ...
    var hhly_city = $(".hhly_cityId").attr('hhly-cityId') || 11959,
        ...
        //url = {
        //    recruitPage: app.baseUrl + '/city/recieve/page',
        //    recruitRecommend:app.baseUrl + '/city/recieve/recommend'
        //},
        p_param = {
            sportType: 0, /*运动类型*/
    ...
    var recruit = {
            listLoad: function () {
                $.get(app.baseUrl + '/city/recieve/page', p_param, function (data) {
                    /*招募数据列表的接口*/
                    //console.log(data, '招募数据列表的接口');
                    if(data.result == 0){
                        app.getId('recruitListCon', data, recruitListWrap);
                    }
                });
            },
            sideLoad: function () {
                $.get(app.baseUrl + '/city/recieve/recommend', r_param, function (data) {
                    /*招募明星球队推荐数据列表的接口*/
                    //console.log(data, '招募明星球队推荐数据列表的接口');
                    if(data.result == 0){
                        app.getId('recruitSideCon', data, recruitSideWrap);
                    }
            ...
            recruitClick: function () {
                ...
                $(".mod_ball").delegate('.moreClick', 'click', function () {
                    ...
                    var that = $(this);
                    cityCommon.appendMore(app.baseUrl + '/city/recieve/page',p_param,'recruitUlCon',recruitUlWrap,that);
                });
                $(".mod_side").delegate('.changeClick', 'click', function () {
                    r_param.page++;
                    r_param.page = r_param.page;
                    var that = $(this);
                    cityCommon.changeMore(app.baseUrl + '/city/recieve/recommend',r_param,'recruitSideCon',recruitSideWrap,that);
                });
