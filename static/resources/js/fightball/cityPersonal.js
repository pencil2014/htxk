...
define(['jquery', 'template', 'sort', 'nav', 'app','cityCommon','blockUI','common','i18n'], function ($, template, sort, nav, app,cityCommon,blockUI) {
    ...
    var hhly_city = $(".hhly_cityId").attr('hhly-cityId') || 11959,
        ...
        //url = {
        //     personalPage: app.baseUrl+'/city/personal/page',
        //     personalRecommend:app.baseUrl+'/city/personal/recommend'
        // },
         p_param = {
             sportType: 0, /*运动类型*/
    ...
    var personal = {
        listLoad: function () {
            $.get(app.baseUrl+'/city/personal/page', p_param, function (data) {
                /*个人数据列表的接口*/
                //console.log(data, '个人数据列表的接口');
                if(data.result == 0){
                    app.getId('personalListCon', data, personalListWrap);    
                }
            });

        },
        sideLoad: function () {
            $.get(app.baseUrl+'/city/personal/recommend', r_param, function (data) {
                /*个人/用户推荐数据列表的接口*/
                //console.log(data, '个人/用户推荐数据列表的接口');
                if(data.result == 0){
                    app.getId('personalSideCon', data, personalSideWrap);    
                }
        ...
        personalClick: function () {
            ...
            $(".mod_ball").delegate('.moreClick', 'click', function () {
                ...
                var that = $(this);
                cityCommon.appendMore(app.baseUrl+'/city/personal/page',p_param,'personalUlCon',personalUlWrap,that);
            });
            $(".mod_side").delegate('.changeClick', 'click', function () {
                r_param.page++;
                r_param.page = r_param.page;
                var that = $(this);
                cityCommon.changeMore(app.baseUrl+'/city/personal/recommend',r_param,'personalSideCon',personalSideWrap,that);
            });
