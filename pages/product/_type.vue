...
<script type="text/javascript">
  ...
  export default {
    ...
    async fetch (context) {
      if (!context.popStateEvent) {
        if (context.query && context.query.goodsCode && context.query.goodsCode.length > 10) {
          context.redirect('/product/detail/' + context.query.goodsCode)
          return
        }
        ...
        } else if (context.params.type === 'match') {
    ...
    methods: {
      // handleTabs () {
      //   this.$router.push({path: '/product', params: {type: this.goodsCategoryCode}, query: {page: 1}})
      // }
    },
    mounted () {
      if (process.browser) {
