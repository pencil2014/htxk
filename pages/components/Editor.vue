...
<script>
  ...
  const toolbars = {
    simple: [['fullscreen', 'source', 'undo', 'redo', 'bold', 'italic', 'underline', 'forecolor', 'justifyleft', 'justifyright', 'justifycenter', 'justifyjustify', 'removeformat', 'simpleupload', 'insertimage']]
  }
  if (process.browser) {
    window.CONST = {
      basePath: ''
  ...
  export default {
    ...
    mounted () {
      ...
      this.$nextTick(() => {
        if (!window.UE) {
          let origin = location.protocol + '//' + location.host.replace('www.', 'org.')
          this.loadScript(origin + '/static/ueditor/ueditor.config.js', () => {
            this.loadScript(origin + '/static/ueditor/ueditor.all.min.js', () => {
              this.loadScript(origin + '/static/ueditor/lang/zh-cn/zh-cn.js', () => {
                this.initEditor()
              })
            })
          })
