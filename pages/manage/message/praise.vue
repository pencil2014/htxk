<template>
  <div>
        <el-breadcrumb>
          ...
          <el-breadcrumb-item>赞</el-breadcrumb-item>
        </el-breadcrumb>
        <div class="content">
            <e-heading grade="1" class="message-title">赞</e-heading>
                      ...
                      <li v-for="(item, index) of messageList.list" :key="index">
                        <e-cellbox top :gutter="20">
                          ...
                          <e-cell-item>
                            ...
                            <div class="message-tex">给你的<span v-if='items.value == item.noticeType' v-for="(items, i) in noticeTypeList" :key="i">{{items.label}}</span>点了赞</div>
                            ...
                            <nuxt-link target='_blank' :to="{path:'/community/detail/' + item.feedId}">
                              <p class="message-com" v-if="item.noticeType == 1">
                                 <span>原帖子：</span><span v-if='item.content'>{{ item.content }}</span><span v-else>无</span>
                              </p>
                              <p class="message-com" v-else>
                                  <span>我的评论：</span><span v-if='item.content'>{{ item.content }}</span><span v-else>无</span>
                              </p>
                            </nuxt-link>
                          </e-cell-item>
                        </e-cellbox>
...
export default {
  ...
  methods: {
    fetch () {
      api.getPraiseListData(this.query)
        .then((res) => {
