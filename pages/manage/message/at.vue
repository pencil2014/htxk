<template>
  <div>
        ...
        <div class="content">
            <e-heading grade="1" class="message-title">@我的</e-heading>
                      ...
                      <li v-for="(item, index) of messageList.list" :key="index">
                        <e-cellbox top :gutter="20">
                          ...
                          <e-cell-item>
                            ...
                            <div class="message-tex">在<span v-if='items.value == item.noticeType' v-for="(items, i) in noticeTypeList" :key="i">{{items.label}}</span>中@了你</div>
                            ...
                            <nuxt-link target='_blank' :to="{path: '/community/detail/' + item.feedId}">
                              <p class="message-com" v-if="item.noticeType == 1">
                                <span>原帖子:</span><span>{{ item.content }}</span>
                              </p>
                              <p class="message-com" v-else>
                                <span>{{ item.userInfoBasicRespVO.nickName }}的评论：</span><span>{{ item.content }}</span>
                              </p>
                              </nuxt-link>
                          </e-cell-item>
                        </e-cellbox>
