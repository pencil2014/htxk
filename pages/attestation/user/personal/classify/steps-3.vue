...
<script>
  ...
  export default {
    ...
    data () {
      return {
        value: this.ipCateIdArray[2],
        list: []
      }
    },
    created () {
      this.handleActive(3)
      if (this.ipCateIdArray[1] && this.ipCateIdArray[1].ipCateId) {
        api.getUserPersonType({parentId: this.ipCateIdArray[1].ipCateId}, {context: this, successMsg: 'none'}).then((json) => {
